...
contract DeployYearnUSDCHorizonStrategyScriptTest is Test {
    ...
    function test_deployYearnUSDCHorizonStrategy_revertsOffBase() public {
        DeployYearnUSDCHorizonStrategyScript.YearnUSDCHorizonDeployConfig memory config = _config(address(targetVault));
        vm.chainId(1);

        vm.expectRevert(abi.encodeWithSelector(BaseERC4626DeploymentScript.InvalidBaseChain.selector, 1));
        deployScript.deployYearnUSDCHorizonStrategy(newOwner, config);
    }

    function test_deployYearnUSDCHorizonStrategy_revertsForAssetMismatch() public {
        TestERC20 otherAsset = new TestERC20(1_000_000e18, 18);
        ERC4626Mock mismatchedVault = new ERC4626Mock(address(otherAsset));
        DeployYearnUSDCHorizonStrategyScript.YearnUSDCHorizonDeployConfig memory config = _config(address(mismatchedVault));

        vm.expectRevert(
            abi.encodeWithSelector(
                BaseERC4626DeploymentScript.DeploymentAssetMismatch.selector, address(mismatchedVault), address(assetToken), address(otherAsset)
            )
        );
        deployScript.deployYearnUSDCHorizonStrategy(newOwner, config);
    }
    ...
    function _config(address vault) internal view returns (DeployYearnUSDCHorizonStrategyScript.YearnUSDCHorizonDeployConfig memory config) {
        IMYTStrategy.StrategyParams memory params = deployScript.defaultParams();
        params.owner = address(deployScript);
        config = DeployYearnUSDCHorizonStrategyScript.YearnUSDCHorizonDeployConfig({myt: address(myt), yearnVault: vault, params: params});
    }
}
