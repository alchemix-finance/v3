...
contract MockRewardsControllerOPWETH {
...
contract MockSwapExecutorOPWETH {
...
contract AaveV3OPWETHStrategyTest is BaseStrategyTest {
    ...
    address public constant REWARD_TOKEN = 0x4200000000000000000000000000000000000042;
    ...
    function createStrategy(address vault, IMYTStrategy.StrategyParams memory params) internal override returns (address) {
        return address(
            new AaveStrategy(vault, params, WETH, AAVE_V3_OP_WETH_ATOKEN, AAVE_V3_OP_POOL_ADDRESS_PROVIDER, REWARDS_CONTROLLER, REWARD_TOKEN)
        );
    }
    ...
    function test_allowlisted_revert_custom_selector_is_deterministic() public {
        ...
        vm.mockCall(AAVE_V3_OP_POOL_ADDRESS_PROVIDER, abi.encodeWithSelector(getPoolSelector), abi.encode(mockPool));
        vm.mockCallRevert(
    ...
    function test_claimRewards_succeeds() public {
        ...
        vm.prank(address(1));
        IMYTStrategy(strategy).claimRewards(AAVE_V3_OP_WETH_ATOKEN, "", 0);
        vm.stopPrank();
    }
    ...
    function test_claimRewards_emits_event_and_vault_receives_asset() public {
        ...
        MockRewardsControllerOPWETH mockRC = new MockRewardsControllerOPWETH(REWARD_TOKEN, rewardAmount);
        vm.etch(REWARDS_CONTROLLER, address(mockRC).code);
        deal(REWARD_TOKEN, REWARDS_CONTROLLER, rewardAmount);

        MockSwapExecutorOPWETH mockSwap = new MockSwapExecutorOPWETH(WETH, mockSwapReturn);
        ...
        vm.prank(address(1));
        uint256 received = IMYTStrategy(strategy).claimRewards(AAVE_V3_OP_WETH_ATOKEN, quote, 4.99e15);
    ...
    function test_aave_v3_opweth_yield_accumulation() public {
        vm.startPrank(allocator);
