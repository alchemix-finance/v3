...
contract SteakhouseUSDCInvariantTest is E2EInvariantStrategyTest {
    ...
    /// @dev The Steakhouse vault's `maxDeposit(address)` always returns 0 even though direct
    ///      `deposit()` calls are accepted without limit (verified on fork). The handler's
    ///      allocate bounds computation relies on `maxDeposit`, so mock the view for the real
    ///      strategy to reflect the vault's actual (unlimited) deposit capacity.
    function setUp() public virtual override {
        super.setUp();
        vm.mockCall(STEAKHOUSE_USDC_VAULT, abi.encodeWithSignature("maxDeposit(address)", realStrategy), abi.encode(type(uint256).max));
    }
