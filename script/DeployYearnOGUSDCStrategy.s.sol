...
contract DeployYearnOGUSDCStrategyScript is BaseERC4626DeploymentScript {
    ...
    function deployYearnOGUSDCStrategy(address targetOwner, YearnOGUSDCDeployConfig memory config) public returns (address strategyAddr) {
        _validateERC4626Deployment(targetOwner, config.myt, config.yearnOGVault, config.params.owner);
    ...
    function run() public returns (address strategyAddr) {
        ...
        _validateBaseAsset(newOwner, config.myt, config.yearnOGVault, config.params.owner, USDC);
